...
A little bit more general is the Herschel-Bulkley Model that extends the Power-law model with a yield stress $\tau_0$. 
In regions where the shear stress $\tau < \tau_0$, the fluid behaves as a non-deformable solid ($\gam=\mathbf{0}$) and is called \textit{unyielded}. In this region, the fluid undergoes a rigid-body motion: a uniform translation and/or a uniform rotation.

Above that threshold, the fluid becomes \textit{yielded} and follows the Power-law model. Bingham fluids correspond to the special case of $n=1$. The constitutive law of the Herschel-Bulkley Model reads as follows
\begin{equation}
    \begin{aligned}
...
In the Poiseuille flow simulation, I obtained different results with different setups of boundary conditions at the inflow and outflow.
\begin{table}[hb]
    \centering
    \begin{tabular}[t]{cccc}
        \toprule
         & & Free velocity & Normal force and tangential velocity\\
        Incompressibility & Integral & & \\
        \midrule
        \multirow{2}{*}{Weak} & Bulk & $4.30 \times 10^{-11}$ & $-4.33 \times 10^{-3}$\\
        & Boundary & $1.01 \times 10^{-10}$ &  $-4.33 \times 10^{-3}$\\
        \multirow{2}{*}{Strong} & Bulk & $5.32 \times 10^{-10}$ & $-4.20 \times 10^{-3}$\\
         & Boundary & $1.02 \times 10^{-9}$ &  $-4.20 \times 10^{-3}$\\
        \bottomrule
    \end{tabular}
    \caption{Deviation from the analytical functional: $\mathcal{J}(u^h) - \mathcal{J}(u)$.}
\end{table}%

On the one hand, when we impose the tangential velocity, the solution corresponds exactly to a Poiseuille flow, uniform in $x$. The numerical result is excellent, whatever the way of imposing the pressure gradient (boundary or volume integral), and the incompressibility condition (weak or strong). In the case of a flow in a channel of 2 meters long and 1 meter wide, the exact energy functional is $-1/12$ and is always obtained numerically within at least $10^{-9}$.

On the other hand, when the inflow/outflow velocity is left free, poor results are obtained, especially near the boundary, where the vertical velocity is no longer zero. The maximum velocity at the channel is also too high $0.135$ instead of $0.125$. Furthermore, the pressure at the corners is very far from its analytical value. The solver manages to find a velocity field with a lower energy functional: $\mathcal{J}(u^h) < \mathcal{J}(u)$ for both weak/strong constraints and body/boundary integrals. This is possible because the incompressibility condition is not respected at every position $(x,y)$ of the domain, but rather in a discrete way either at the gauss points, or via pressure test functions.

The two situations are represented graphically in \cref{fig:poiseuille_vzero,fig:poiseuille_free}, with the pressure and velocity fields. The horizontal and vertical velocity profiles are also provided at the inflow/outflow.

\begin{figure}
    \centering
    \includesvg[width=\textwidth]{../figures/poiseuille_vzero_weak.svg}
    \caption{Poiseuille flow with imposed zero vertical velocity at the inflow/outflow.}
    \label{fig:poiseuille_vzero}
\end{figure}
\begin{figure}
    \centering
    \includesvg[width=\textwidth]{../figures/poiseuille_free_weak.svg}
    \caption{Poiseuille flow with free velocity at the inflow/outflow.}
    \label{fig:poiseuille_free}
\end{figure}
